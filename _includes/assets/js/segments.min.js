...
inputs.updateBinaryHex();},rebuildHighlights:function(index){var ids=['DP','G','F','E','D','C','B','A'],$letters=$('#inputsLetters .view'),$segs=$('#segRail .seg-display').slice(index,index+4);console.log($segs.get());console.log($segs.get(0));$segs.each(function(i){var $seg=$(this),$letter=$letters.eq(i).find('.letter');$letter.removeClass('on');ids.forEach(function(id){var $path=$seg.find('[data-segment="'+id+'"]');if(svg.hasClass($path,'on')){$letter.filter('[data-letter="'+id+'"]').addClass('on');}});});inputs.updateBinaryHex();},updateBinaryHex:function(){var $letters=$('#inputsLetters .view'),$bins=$('#inputsBinary .view'),$hexs=$('#inputsHex .view');$letters.each(function(i){var $this=$(this),$bin=$bins.eq(i),$hex=$hexs.eq(i),binStr='0b',hexStr='0x';$this.find('.letter').each(function(){if($(this).hasClass('on')){binStr+=((settings.activeLow===1)?'0':'1');}else{binStr+=((settings.activeLow===1)?'1':'0');}});hexStr+=('00'+(binStr*1).toString(16)).slice(-2);console.log(binStr);console.log(hexStr);$bin.text(binStr);$hex.text(hexStr);});}},svg={addClass:function($elem,toAdd){if(svg.hasClass($elem,toAdd)){return;}
