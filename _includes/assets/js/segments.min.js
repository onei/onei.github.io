...
(function($){'use strict';var disabled=false,x={offset:null,start:null,first:null,last:null},settings={activeLow:false,order:1,},maxElements=4,init={main:function(){init.setDimensions();init.setupSegments();init.setColour();init.enforceSettings();$('#segRail .seg').click(segments.highlight);$('#chevronLeft').click(segments.moveLeft);$('#chevronRight').click(segments.moveRight);$('#optionsColours .pick').click(options.colourPick);$('#inputsLetters .letter').click(inputs.highlightLetter);},setDimensions:function(){var maxWidth=$('#article').width(),overlayWidth=$('#segOverlayLeft').width(),segmentWidth=$('#segRail .seg-display').width(),separatorWidth=(maxWidth-(2*overlayWidth)-(segmentWidth*4))/3;x.offset=segmentWidth+separatorWidth;x.start=overlayWidth;console.log('xstart, xOffset:',x.start,x.offset);},setupSegments:function(){var $segs=$('#segRail .seg-display');$segs.each(function(index){var $this=$(this),viewIndex=index-2,xPosition=x.start+(viewIndex*x.offset),css='translateX('+xPosition+'px)';$this.css('transform',css);if(viewIndex>=0&&viewIndex<4){$this.attr('data-view',viewIndex);}
if(index===0){x.first=xPosition;}else if(index===7){x.last=xPosition;}});},setColour:function(){$('#coloursPickGreen').addClass('selected');$('#segRail, #inputsLetters').addClass('green');},enforceSettings:function(){var $letters=$('#inputsLetters .view');if(settings.order===0){$letters.each(function(){var $this=$(this),$letters=$this.children();$this.append($letters.get().reverse());});}
inputs.updateBinaryHex();}},segments={moveLeft:function(){if(disabled){return;}
disabled=true;setTimeout(function(){disabled=false;},550);var $segs=$('#segRail .seg-display'),re=/matrix\(\s*-?\d+,\s*-?\d+,\s*-?\d+,\s*-?\d+,\s*(-?\d+),\s*(-?\d+)\s*\)/,viewFirst;$segs.each(function(){var $this=$(this),transform=$this.css('transform'),matches=transform.match(re),xPosition,css;if(matches===null){console.log('Error: No matches found.');return;}
...
disabled=true;setTimeout(function(){disabled=false;},550);var $segs=$('#segRail .seg-display'),re=/matrix\(\s*-?\d+,\s*-?\d+,\s*-?\d+,\s*-?\d+,\s*(-?\d+),\s*(-?\d+)\s*\)/,viewFirst;$segs.each(function(){var $this=$(this),transform=$this.css('transform'),matches=transform.match(re),xPosition,css;if(matches===null){console.log('Error: No matches found.');return;}
xPosition=matches[1]*1;xPosition+=x.offset;css='translateX('+xPosition+'px)';$this.css('transform',css);});viewFirst=$segs.filter('[data-view="0"]').attr('data-index');viewFirst=(viewFirst*1)-1;console.log(viewFirst);segments.reView(viewFirst);segments.prepend();if($segs.length<$('#segRail .seg-display').length){console.log('extra element detected');viewFirst+=1}
...
inputs.updateBinaryHex();}},options={colourPick:function(){var $this=$(this),$old=$('#optionsColours .selected'),$update=$('#segRail, #inputsLetters'),newClass=$this.attr('data-colour'),oldClass=$old.attr('data-colour');$old.removeClass('selected');$this.addClass('selected');$update.removeClass(oldClass).addClass(newClass);}},inputs={highlightLetter:function(){var $this=$(this),$view=$this.parent('.view'),letter=$this.attr('data-letter'),viewIndex=$view.attr('data-view'),$segment=$('#segRail .seg-display').filter('[data-view="'+viewIndex+'"]').find('.seg').filter('[data-segment="'+letter+'"]');if($this.hasClass('on')){$this.removeClass('on');svg.removeClass($segment,'on');}else{$this.addClass('on');svg.addClass($segment,'on');}
inputs.updateBinaryHex();},rebuildHighlights:function(index){var ids=['DP','G','F','E','D','C','B','A'],$letters=$('#inputsLetters .view'),$segs=$('#segRail .seg-display').slice(index,index+4);console.log($segs.get());console.log($segs.get(0));$segs.each(function(i){var $seg=$(this),$letter=$letters.eq(i).find('.letter');$letter.removeClass('on');ids.forEach(function(id){var $path=$seg.find('[data-segment="'+id+'"]');if(svg.hasClass($path,'on')){$letter.filter('[data-letter="'+id+'"]').addClass('on');}});});inputs.updateBinaryHex();},updateBinaryHex:function(){var $letters=$('#inputsLetters .view'),$bins=$('#inputsBinary .view'),$hexs=$('#inputsHex .view');$letters.each(function(i){var $this=$(this),$bin=$bins.eq(i),$hex=$hexs.eq(i),binStr='0b',hexStr='0x';$this.find('.letter').each(function(){if($(this).hasClass('on')){binStr+=(settings.activeLow?'0':'1');}else{binStr+=(settings.activeLow?'1':'0');}});hexStr+=('00'+(binStr*1).toString(16)).slice(-2);console.log(binStr);console.log(hexStr);$bin.text(binStr);$hex.text(hexStr);});}},svg={addClass:function($elem,toAdd){if(svg.hasClass($elem,toAdd)){return;}
